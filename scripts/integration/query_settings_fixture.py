...
def main():
    ...
    parser.add_argument(
        "binary", type=Path, help="Native query_settings_corruption_test binary"
    )
    args = parser.parse_args()
    binary = args.binary.resolve(strict=True)
    schema = (
        Path(__file__).resolve().parents[2] / "crates/storage/src/schema.sql"
    ).read_text()
    cases = {
        "malformed": "{broken query preferences",
        "out_of_range": json.dumps(
            {"version": 1, "page_size": 99, "timeout_seconds": 0}
        ),
    }
    for name, corrupt in cases.items():
        with tempfile.TemporaryDirectory(
            ...
            subprocess.run(
                [
                    str(binary),
                    "corruptSettingsKeepQueriesUsableAndRequireExplicitRepair",
                ],
                env=environment,
                check=True,
                timeout=90,

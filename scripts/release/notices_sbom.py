...
QT_PLUGINS = {"platforms/libqcocoa.dylib", "styles/libqmacstyle.dylib",
...
def normal_dependency_packages(metadata, root_package):
    ...
    roots = [row["id"] for row in metadata["packages"]
...
def generate(stage, cargo_metadata, qt_notices, qt_version, source_candidate,
             cargo_licenses, output, release_id, created, cargo_root_package, cargo_target):
    ...
    if source.get("source_kind") == "working-tree snapshot" and not release_id.startswith("candidate-"):
        raise ValueError("Working-tree snapshots require a candidate- release identifier")
    manifest_bytes = read_local(stage, "manifest.json")
    if read_local(stage, "SHA256SUMS").decode().strip() != digest(manifest_bytes) + "  manifest.json":
        raise ValueError("Stage manifest checksum mismatch")
    manifest = json.loads(manifest_bytes)
    stage_source = manifest.get("source_candidate", {})
    if (stage_source.get("manifest_sha256") != digest(source_bytes) or
            stage_source.get("source_kind") != source.get("source_kind") or
            stage_source.get("git_revision") != source.get("git_revision")):
    ...
    for p in stage.rglob("*"):
        if p.is_symlink() and (p.readlink().is_absolute() or not p.resolve(strict=True).is_relative_to(stage.resolve())):
    ...
    for name, module in modules.items():
        if qt_version != "6.8.3" or module.get("version") != qt_version or module.get("sha256") != QT_SOURCE_HASHES.get(name) or name not in QT_SOURCE_HASHES:
    ...
    versions = {plistlib.loads(data).get("CFBundleShortVersionString") for name, data in payload.items()
    ...
    qsci_records = [name for name in payload if name.endswith("/licenses/QScintilla/source.json")]
    ...
    app_licenses = [name for name in payload if name.endswith("/Resources/licenses/LICENSE")]
    ...
    def package(key, name, version, license_expression, license_texts, url=None):
        if not isinstance(version, str) or not version or not isinstance(license_expression, str) or not license_expression.strip():
        ...
        row = {"SPDXID": package_id, "name": name, "versionInfo": version,
               "downloadLocation": url or "NOASSERTION", "filesAnalyzed": False,
               "licenseConcluded": "NOASSERTION", "licenseDeclared": license_expression,
               "copyrightText": "NOASSERTION", "comment": "Included license texts: " + ", ".join(paths)}
        packages.append(row)
        package_ids[key] = package_id
        return package_id

    app_id = package("choscordb", "ChoscorDB", release_id, "GPL-3.0-or-later",
                     {"LICENSE": payload[app_licenses[0]]})
    packages[-1]["sourceInfo"] = "Source candidate manifest SHA256: " + digest(source_bytes)
    qt_id = package("qt", "Qt", qt_version, qt_record.get("license"), qt_texts, qt_record.get("url"))
    qsci_id = package("qscintilla", "QScintilla", qsci_record.get("version"), qsci_record.get("license"),
                      text_files(stage / PurePosixPath(qsci_path).parent), qsci_record.get("url"))
    ...
    for row in selected:
        ...
        if any(record.get(key) != row.get(key) for key in ["name", "version", "license", "source"]):
            raise ValueError("Cargo license package metadata mismatch")
        package_id = package("cargo:" + row["id"], row["name"], row["version"], row.get("license"),
                             checked_texts(cargo_licenses / coordinate, record["files"]), row.get("repository") or row.get("homepage"))
        cargo_ids.append(package_id)
        if any("proc-macro" in target.get("kind", []) for target in row.get("targets", [])):
    ...
    for row in selected:
        if row["name"] != "libsqlite3-sys":
            continue
        node = next(node for node in metadata["resolve"]["nodes"] if node["id"] == row["id"])
        features = set(node.get("features", []))
        if "bundled" not in features or any("sqlcipher" in feature for feature in features):
        ...
        def define(data, name, pattern):
            matches = re.findall(rb'^#define\s+' + name.encode() + rb'\s+"(' + pattern + rb')"', data, re.MULTILINE)
        ...
        source_id = define(header, "SQLITE_SOURCE_ID", rb'\d{4}-\d\d-\d\d \d\d:\d\d:\d\d [0-9a-f]{64}')
        if define(amalgamation, "SQLITE_VERSION", rb'\d+\.\d+\.\d+') != version or define(amalgamation, "SQLITE_SOURCE_ID", rb'\d{4}-\d\d-\d\d \d\d:\d\d:\d\d [0-9a-f]{64}') != source_id:
            raise ValueError("SQLite header/amalgamation provenance mismatch")
        blessing = re.search(rb'The author disclaims copyright.*?May you share freely, never taking more than you give\.', amalgamation, re.DOTALL)
        if not blessing or b"May you do good and not evil." not in blessing[0] or b"May you find forgiveness for yourself and forgive others." not in blessing[0]:
            raise ValueError("SQLite blessing license is missing")
        license_text = re.sub(rb'\n\*\* ?', b'\n', blessing[0]) + b"\n"
        sqlite_id = package("native:sqlite:" + source_id, "SQLite", version, "blessing",
                            {"LICENSE": license_text}, "https://www.sqlite.org/src/info/" + source_id.split()[-1])
        packages[-1]["sourceInfo"] = "Bundled libsqlite3-sys source; SQLITE_SOURCE_ID " + source_id + "; sqlite3.h SHA256 " + digest(header) + "; sqlite3.c SHA256 " + digest(amalgamation)
        sqlite_relationships += [{"spdxElementId": app_id, "relationshipType": "STATIC_LINK", "relatedSpdxElement": sqlite_id},
                                 {"spdxElementId": package_ids["cargo:" + row["id"]], "relationshipType": "DEPENDS_ON", "relatedSpdxElement": sqlite_id}]

    files, relationships = [], [{"spdxElementId": "SPDXRef-DOCUMENT", "relationshipType": "DESCRIBES", "relatedSpdxElement": app_id}]
    ...
    for package_id in cargo_ids:
        if package_id in proc_macro_ids:
            relationships.append({"spdxElementId": package_id, "relationshipType": "BUILD_DEPENDENCY_OF", "relatedSpdxElement": app_id})
        else:
            relationships.append({"spdxElementId": app_id, "relationshipType": "STATIC_LINK", "relatedSpdxElement": package_id})
    for name, data in sorted(payload.items()):
        ...
        if "qscintilla" in name.lower():
            owner = qsci_id
        elif re.search(r"/Qt[^/]+\.framework/", name) or "/PlugIns/" in name or name.endswith("/qt.conf") or "/licenses/Qt/" in name:
            owner = qt_id
        elif "/Frameworks/" in name or re.search(r"\.(dylib|dll|so)(\.|$)", name, re.IGNORECASE):
            raise ValueError("Unknown third-party runtime ownership: " + name)
        else:
            owner = app_id
        files.append({"SPDXID": file_id, "fileName": "./" + name,
    ...
    document = {"spdxVersion": "SPDX-2.3", "dataLicense": "CC0-1.0", "SPDXID": "SPDXRef-DOCUMENT",
                "name": "ChoscorDB " + release_id, "documentNamespace": "urn:uuid:" + str(uuid.uuid5(uuid.NAMESPACE_URL, release_id + ":" + digest(manifest_bytes))),
                "creationInfo": {"created": created, "creators": ["Tool: choscordb-notices-sbom"]},
                "documentComment": "Operating-system libraries are external prerequisites. Cargo normal-dependency closure rooted at " + cargo_root_package + "; caller supplied --filter-platform metadata for " + cargo_target + ". Dev/build-only and unreachable packages excluded.",
                "packages": sorted(packages, key=lambda p: p["SPDXID"]), "files": files,
                "relationships": sorted(relationships, key=lambda r: (r["spdxElementId"], r["relationshipType"], r["relatedSpdxElement"]))}
    notices = ["# Third-party notices", "", "Release identifier: " + release_id, "",
               "Operating-system libraries are external prerequisites. Cargo entries cover normal dependencies of " + cargo_root_package + " for " + cargo_target + ".", ""]
    for row in sorted(packages, key=lambda p: (p["name"], p["versionInfo"])):
        if row["SPDXID"] == app_id:
            continue
        notices += ["## " + row["name"] + " " + row["versionInfo"], "",
                    "Declared license: " + row["licenseDeclared"], "",
                    "Source: " + row["downloadLocation"], "", row["comment"], ""]
    artifacts = {**license_outputs, "THIRD_PARTY_NOTICES.md": "\n".join(notices).encode(),
                 "sbom.spdx.json": (json.dumps(document, indent=2, sort_keys=True) + "\n").encode()}
    ...
    try:
        ...
        (output / "SHA256SUMS").write_text("".join(digest(data) + "  " + name + "\n" for name, data in sorted(artifacts.items())))
...
def main():
    ...
    for name in ["stage", "cargo-metadata", "qt-notices", "source-candidate", "cargo-licenses", "output"]:
        parser.add_argument("--" + name, required=True, type=Path)
    for name in ["qt-version", "release-id", "created", "cargo-root-package", "cargo-target"]:
        parser.add_argument("--" + name, required=True)
    args = parser.parse_args()
    generate(args.stage, args.cargo_metadata, args.qt_notices, args.qt_version,
             args.source_candidate, args.cargo_licenses, args.output, args.release_id, args.created,
             args.cargo_root_package, args.cargo_target)

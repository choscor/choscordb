...
class CargoLicenseTest(unittest.TestCase):
    ...
    def fixture(self, missing_license=False):
        ...
        document = {
            ...
            "packages": [app, dependency, unused],
            "resolve": {
                "nodes": [
                    {
                        "id": "app",
                        "deps": [{"pkg": "dependency", "dep_kinds": [{"kind": None}]}],
                    },
                    {"id": "dependency", "deps": []},
                    {"id": "unused", "deps": []},
                ]
    ...
    def test_collects_only_normal_closure_with_deterministic_index(self):
        self.fixture()
        first = cargo_licenses.collect(
            self.metadata, self.fallback, self.root / "first", "app"
        )
        second = cargo_licenses.collect(
            self.metadata, self.fallback, self.root / "second", "app"
        )
        self.assertEqual(
            (first / "index.json").read_bytes(), (second / "index.json").read_bytes()
        )
    ...
    def test_missing_package_license_requires_coordinate_bound_fallback(self):
        ...
        with self.assertRaisesRegex(ValueError, "unavailable"):
            cargo_licenses.collect(
                self.metadata, self.fallback, self.root / "output", "app"
            )
        ...
        (override / "source.json").write_text(
            json.dumps(
                {
                    "coordinate": "dependency@1.0",
                    "url": "https://example.invalid/license",
                    "files": {"LICENSE": hashlib.sha256(license_data).hexdigest()},
                }
            )
        )
        output = cargo_licenses.collect(
            self.metadata, self.fallback, self.root / "output", "app"
        )
        self.assertEqual(
            json.loads((output / "index.json").read_text())["packages"][0]["origin"],
            "verified-fallback",
        )
        (override / "LICENSE").write_text("tampered fallback")
        with self.assertRaisesRegex(ValueError, "fallback digest"):
            cargo_licenses.collect(
                self.metadata, self.fallback, self.root / "tampered", "app"
            )
    ...
    def test_empty_and_symlinked_license_sources_fail_closed(self):
        ...
        with self.assertRaisesRegex(ValueError, "Empty"):
            cargo_licenses.collect(
                self.metadata, self.fallback, self.root / "empty", "app"
            )
        license_path.unlink()
        ...
        with self.assertRaisesRegex(ValueError, "unavailable"):
            cargo_licenses.collect(
                self.metadata, self.fallback, self.root / "link", "app"
            )

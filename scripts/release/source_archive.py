...
ROOT_FILES = (".clang-format", ".gitignore", "CMakeLists.txt", "CMakePresets.json",
              "Cargo.lock", "Cargo.toml", "LICENSE", "README.md", "deny.toml",
              "rust-toolchain.toml")
EXCLUDED_DIRECTORIES = {"build", "target", ".git", "__pycache__", ".venv", "venv",
                        ".pytest_cache", ".mypy_cache", ".ruff_cache", ".cache", "node_modules",
                        "CMakeFiles"}
EXCLUDED_SUFFIXES = (".png", ".log", ".key", ".pem", ".p12", ".pfx",
                     ".pyc", ".pyo", ".sqlite", ".sqlite3", ".db", ".sqlite-wal",
                     ".sqlite-shm", ".sqlite-journal", ".sqlite3-wal", ".sqlite3-shm",
                     ".sqlite3-journal", ".db-wal", ".db-shm", ".db-journal",
...
def excluded_file(name):
    lower = name.lower()
    return (lower.endswith(EXCLUDED_SUFFIXES) or lower in {
                ".ds_store", ".env", "cmakecache.txt", "cmake_install.cmake",
                "compile_commands.json", "install_manifest.txt"}
            or ".so." in lower or lower.startswith(".env.")
...
def read_source(path):
    ...
    if os.open not in os.supports_dir_fd or not hasattr(os, "O_NOFOLLOW"):
        raise ValueError("Secure source traversal requires descriptor-relative no-follow opens")
        ...
        for directory, directories, names in os.walk(
            ...
            for child in directories + names:
                if (parent / child).is_symlink():
                    raise ValueError(f"Symlink in source tree: {(parent / child).relative_to(root)}")
            ...
            for child in names:
                ...
                if not stat.S_ISREG(path.stat().st_mode):
                    raise ValueError(f"Non-regular source file: {path.relative_to(root)}")
                files.append(path)
    for path in files:
        relative = path.relative_to(root).as_posix()
        if "\\" in relative or ":" in relative or any(ord(char) < 32 for char in relative):
            raise ValueError(f"Source path is unsafe for portable extraction: {relative!r}")
...
def create_candidate(root, output):
    ...
    if output == root or any(output.is_relative_to(root / name) for name in SOURCE_ROOTS):
        raise ValueError("Output must be outside all included source trees")
    files = source_files(root)
    manifest = {"format_version": 1, "source_kind": "working-tree snapshot",

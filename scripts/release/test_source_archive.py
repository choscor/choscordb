...
class SourceArchiveTest(unittest.TestCase):
    ...
    def test_independent_manifest_checksums_and_reproducibility(self):
        ...
        with tarfile.open(first / "choscordb-source.tar.gz", "r:gz") as archive:
            ...
            self.assertEqual(set(expected), {m.name.removeprefix("choscordb-source/") for m in members})
    ...
    def test_excludes_local_artifacts_but_preserves_build_script(self):
        for path in ["build/binary", "target/binary", ".git/config", "crates/target/binary",
                     "scripts/__pycache__/a.pyc", "scripts/.venv/private", "docs/local.sqlite",
                     "docs/local.sqlite-wal", "docs/local.db-shm", "docs/file~",
                     "desktop/file.cpp.swp", "scripts/.env", "scripts/.env.local",
                     "docs/screenshot.PNG", "scripts/install.log", "scripts/install.LOG",
                     "scripts/private.key", "scripts/private.pem", "scripts/private.p12",
                     "scripts/private.pfx", "secret.txt"]:
            file = self.root / path
            file.parent.mkdir(parents=True, exist_ok=True)
            file.write_text("excluded")
        manifest = json.loads((self.generate() / "manifest.json").read_text())
        self.assertFalse(any("excluded" in (self.root / row["path"]).read_text()
    ...
    def test_ancestor_replaced_after_discovery_never_archives_external_bytes(self):
        ...
        with patch.object(source_archive, "git_revision", side_effect=replace_directory):
    ...
    def test_compiled_artifacts_are_excluded_inside_source_trees(self):
        for name in ["file.o", "file.obj", "lib.a", "file.lib", "lib.so", "lib.so.1.2",
                     "lib.dylib", "file.dll", "file.exe", "file.pdb", "FILE.DLL",
        ...
        self.assertEqual([entry["path"] for entry in entries if entry["path"].startswith("desktop/")],
                         ["desktop/example.txt"])
    ...
    def test_cmake_build_outputs_excluded_but_cmake_sources_preserved(self):
        artifacts = ["CMakeFiles/compiler.txt", "cmake-build-debug/generated.txt",
                     "cmake-build-release/generated.txt", "CMakeCache.txt",
                     "cmake_install.cmake", "compile_commands.json", "install_manifest.txt"]
        ...
        paths = {entry["path"] for entry in
                 json.loads((self.generate() / "manifest.json").read_text())["files"]}
    ...
    def test_git_head_is_recorded_without_claiming_clean_committed_contents(self):
        ...
        self.assertEqual(license_entry["sha256"],

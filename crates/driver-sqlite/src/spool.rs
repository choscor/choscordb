...
impl Spool {
    ...
    pub fn load(&mut self, handle: Handle) -> Result<Value> {
        let first = self.reader.read_chunk(handle, 0, MAX_VALUE_CHUNK_BYTES)?;
        if first.total_bytes > 64 * 1024 * 1024 {
            return Err(DriverError::new(
                ErrorKind::ResourceLimit,
                "Value exceeds 64 MiB detail-view limit",
            ));
        }
        let kind = first.kind;
        let total = first.total_bytes as usize;
        ...
        if kind == DeferredKind::Text {
            String::from_utf8(bytes)
                .map(Value::Text)
                .map_err(|_| DriverError::new(ErrorKind::Query, "Invalid UTF-8 in SQLite text"))
        } else {
